...
- **Global settings** (#6, #10): `~/.dangerous_claude/config.json` supplies defaults for every
  project — `writeable_home`, `project_readonly`, `always_use_dangerous_skip_permissions`,
  `tools`, `extra_write_paths`, `readable_paths` — merged with per-project config instead of
...
- **No more hardcoded lab paths** (#7, #11): the project config file is now
  ...
  The `tools` feature's `TOOLS_ROOT` and the system prompt's lab-specific doc/wiki pointers are
  now configurable (`tools_root`, `system_prompt_note`) instead of hardcoded.
