#!/usr/bin/env bash
# Usage: bash start.sh [/abs/path/to/project] [-- extra claude args...]
...
#--env    "AWS_PROFILE=central-claude-access-dev-us-east-1" \
...
# fall back to a plain local claude session if we can't write to the project
# dir, or a config file already exists there but isn't writable
if [[ ! -w "${WORK_DIR}" ]] || { [[ -e "${CONFIG_FILE}" ]] && [[ ! -w "${CONFIG_FILE}" ]]; }; then
    echo "No write permission for ${WORK_DIR} (or its .agentic_peer_project.json) — starting claude locally instead of in the container."
    exec claude "$@"
fi
